...
- 既存の証明書の自動バックアップ
- インストール前にダウンロードしたPEMバンドルを検証
- 一時ファイル経由で証明書をアトミックにインストール
- インストール後の失敗時にバックアップから自動ロールバック
- 証明書ハッシュリンクの再構築
- SSL接続テスト機能
- 現在の証明書状態の確認
...
3. ダウンロードしたバンドルを検証（PEMマーカーとサイズ制限）
4. 一時ファイル (`/etc/ssl/certs/ca-certificates.crt.tmp`) に書き込み、本番ファイルをアトミックに置換
5. ファイル権限を設定 (644)
6. 証明書ハッシュリンクを再構築
7. SSL接続テストを実行

証明書の置換後に失敗した場合（権限設定の失敗など）は、バックアップから自動的に復元します。自動ロールバックに失敗した場合は、手動で復元してください:
